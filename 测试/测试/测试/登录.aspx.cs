        ...
        protected void login_btn_Click(object sender, EventArgs e)
        {
            ...
            if (dr.HasRows)
            {
                ...
                if (str_password == true_psw)
                {
                    ...
                    Response.Write("<script>alert('密码不正确！');location='登录.aspx'</script>");


            }
            else
                Response.Write("<script>alert('登录失败，请检查用户名或密码是否输入正确！');location='登录.aspx'</script>");
